  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 1, marginTop: 4 }}>
        {Array.from({ length: concealedCount }).map((_, i) => <TileBack key={i} size={36} />)}
      </div>
      {exposed.length > 0 && (
        <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {exposed.map((m, i) => (
            <span key={i} style={{ display: 'inline-flex', gap: 1 }}>
              {meldTiles(m).map((t, j) => <TileFace key={j} tile={t} size={36} />)}
            </span>
          ))}
        </div>
      )}
      {flowers.length > 0 && (
        <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
          {flowers.map((f, i) => <TileFace key={i} tile={f} size={36} />)}
        </div>
      )}
    </div>
  );
